...
export async function setFindingsStatus(input: {
  ...
  if (input.status === "complete") {
    const [findingCountResult, summaryResult] = await Promise.all([
      supabase
        .from("findings")
    ...
    if (findingCountResult.error) {
      return { ok: false, message: findingCountResult.error.message };
    }
    if (summaryResult.error) {
      return { ok: false, message: summaryResult.error.message };
    }

    const summary = summaryResult.data;
    const hasSelectedCategory = Boolean(
      summary?.subterranean_termites
      || summary?.drywood_termites
      || summary?.fungus_dryrot
      || summary?.other_findings
      || summary?.further_inspection,
    );
    if ((findingCountResult.count ?? 0) > 0 && !hasSelectedCategory) {
      return {
        ok: false,
