...
export function normalizeWdoAddress(
  ...
  const structuredBuildingNumber = normalizeWdoWhitespace(source.buildingNumber);
  const structuredStreetName = normalizeWdoWhitespace(source.streetName);
  const structuredUnit = normalizeWdoWhitespace(source.unitOrSuite);
  ...
  const canonicalBuildingNumber = structuredBuildingNumber || canonicalMatch?.[1] || "";
  const canonicalStreet = structuredStreetName || canonicalMatch?.[2] || streetLine1;
  const canonicalUnit = structuredUnit || streetLine2;
  const buildingNumber = overrideBuildingNumber || canonicalBuildingNumber;
  const street = overrideStreet
    || [canonicalStreet, canonicalUnit].filter(Boolean).join(" ");
  ...
  if (!structuredBuildingNumber && !canonicalMatch && !overrideBuildingNumber) {
    issues.push({
