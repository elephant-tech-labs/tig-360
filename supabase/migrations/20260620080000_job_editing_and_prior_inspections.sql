...
create index if not exists inspection_jobs_prior_job_id_idx
on public.inspection_jobs (prior_job_id);
...
create or replace function public.create_inspection_job(
  ...
  prior_inspection_job_id uuid default null
)
returns uuid
language plpgsql
set search_path = ''
...
begin
  ...
  if inspection_report_type in ('supplemental', 'reinspection') and prior_inspection_job_id is null then
    raise exception 'A prior inspection is required for supplemental and reinspection reports';
  end if;

  if inspection_report_type in ('complete', 'limited') and prior_inspection_job_id is not null then
    raise exception 'Only supplemental and reinspection reports may reference a prior inspection';
  end if;
  ...
  if prior_inspection_job_id is not null then
    ...
    where job.id = prior_inspection_job_id
      and job.organization_id = target_organization_id;

    if new_property_id is null then
      raise exception 'Prior inspection not found';
    end if;

    update public.properties
    set
      street_line_1 = trim(property_street_line_1),
      street_line_2 = nullif(trim(property_street_line_2), ''),
      city = trim(property_city),
      region = upper(trim(property_region)),
      postal_code = trim(property_postal_code),
...
create or replace function public.update_inspection_job(
  ...
  inspection_date timestamptz,
  prior_inspection_job_id uuid default null,
  job_internal_notes text default null
)
returns void
language plpgsql
set search_path = ''
...
begin
  ...
  if prior_inspection_job_id = target_job_id then
    raise exception 'A job cannot reference itself as its prior inspection';
  end if;
  ...
  if prior_inspection_job_id is not null then
    if exists (
      with recursive prior_chain as (
        select job.id, job.prior_job_id
        from public.inspection_jobs job
        where job.id = prior_inspection_job_id
          and job.organization_id = target_organization_id
        ...
        select parent.id, parent.prior_job_id
        from public.inspection_jobs parent
      ...
      raise exception 'Prior inspection relationships cannot form a circular chain';
    end if;

    select job.property_id
    into prior_property_id
    from public.inspection_jobs job
    where job.id = prior_inspection_job_id
      and job.organization_id = target_organization_id;

    if prior_property_id is null then
      raise exception 'Prior inspection not found';
    end if;
  ...
  set
    ...
    prior_job_id = prior_inspection_job_id,
    report_type = inspection_report_type,
    inspection_at = inspection_date,
    internal_notes = nullif(trim(job_internal_notes), ''),
    status = case
      when status in ('draft'::public.job_status, 'scheduled'::public.job_status)
        then case
          when inspection_date is null then 'draft'::public.job_status
          else 'scheduled'::public.job_status
        end
...
revoke all on function public.update_inspection_job(
  uuid, uuid, text, text, text, text, text, text, text, timestamptz, uuid, text
) from public;
